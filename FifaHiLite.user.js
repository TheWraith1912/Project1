...
(function() {
    ...
    function loadPlayerDataFromCsv() {
        ...
        $.get( "https://raw.githubusercontent.com/TheWraith1912/Project1/main/iconsxb.csv",
			function( CSVdata) {
    ...
    function getPlayerDataFromSite(){
        for(var i=0;i<=19;i++){
			...
			if(sitePlayerPriceAsNumber <= parseInt(excelDataSheetPrice*1.03)){
				console.log('**' + fullName + ' IS BLUE NOW**')
                $('.name:eq('+i+')').parent().css('background-color','blue');
            }
            ...
            if(sitePlayerPriceAsNumber <= parseInt(excelDataSheetPrice)){
				console.log('**' + fullName + ' IS GRAY NOW**')
                $('.name:eq('+i+')').parent().css('background-color','gray');
            }
